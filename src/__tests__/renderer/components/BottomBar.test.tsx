  ...
  it('shows agent status summary', () => {
    const Wrapper = () => {
      ...
      const agents: AgentState[] = [
        ...
        { id: '3', name: 'c', role: 'r', avatar: 'robot-3', color: '#45B7D1', status: 'idle', needsInput: false, lastActivity: Date.now() },
        { id: '4', name: 'd', role: 'r', avatar: 'circuit', color: '#96CEB4', status: 'waiting', needsInput: true, lastActivity: Date.now() }
      ]
  ...
  it('shows last activity timestamp', () => {
    ...
    const Wrapper = () => {
      ...
      dispatch({
        type: 'ADD_AGENT',
        payload: {
          id: '1', name: 'a', role: 'r', avatar: 'robot-1',
          color: '#FF6B6B', status: 'running', needsInput: false,
          lastActivity: now
        }
      })
      return <BottomBar />
